...
- Repo-wide assistant behavior is tool-agnostic:
  the same communication and decision rules must apply to `Codex`, `GitHub Copilot`, `Claude`, and other assistants used in this project.
...
- Runtime task brief contract (`oap.agent_tasks.task_brief.context_package`, backward-compatible extension):
  ...
  - `collaboration_plan`:
    ...
    `strategy`, `reuse_candidates[]`, `created_profiles[]`, `spawned_instances[]`,
    `orchestration_budget`, optional `delegation_depth`;
  - `ab_test_plan`:
