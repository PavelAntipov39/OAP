...
- Host-specific agent adapters are generated by `scripts/export_host_agents.py` directly from
  `docs/subservices/oap/agents/*/OPERATING_PLAN.md` frontmatter.
  Any host catalog/index artifact is compatibility-only and non-authoritative.
...
- Runtime task brief contract (`oap.agent_tasks.task_brief.context_package`, backward-compatible extension):
  ...
  - `collaboration_plan`:
    ...
    `merge_strategy`, `conflict_policy`, optional `routing_decision`,
  ...
  - Coordinator routing decision package (`collaboration_plan.routing_decision`, backward-compatible extension):
    `route_id`, `task_class`, `primary_executor_agent_id`, `process_agents[]`,
    `why_this_route`, `expected_gain`, `verify_owner_agent_id`,
    `fallback_route`, `considered_routes[]`, optional `decision_confidence`,
    optional `comparison_basis`, optional `comparison_metrics[]`.
    V1 rule:
    the coordinator must compare at least `single_agent_path` vs `delegated_path` before choosing a multi-agent route.
    If the expected gain is weak or unproven, the default path is the simpler route.
...
- Coordinator self-improvement telemetry rule:
  - routing-centric lessons are first-class outcomes;
  - wrong executor choice, over-orchestration, under-orchestration, missing verify owner,
    or unjustified budget spend must produce comparable evidence for future routing decisions;
  - comparison basis for coordinator learning is `baseline route vs selected route` on the same task class,
    using routing accuracy as north-star and quality/cost guardrails before promoting a routing rule.
