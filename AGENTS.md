...
- Глобальные skills, которые используются в ОАП и попадают в generated manifests/индексы, должны быть repo-aware: нельзя хардкодить пути исторических проектов; при отсутствии нужных файлов skill обязан явно сообщать, что текущий workspace не содержит required contract, а generated-артефакты после правки skill должны пересобираться, а не патчиться вручную.
