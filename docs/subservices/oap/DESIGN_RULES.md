...
- Для обзорных текстов, section descriptions, tooltip и modal text обязательна отдельная редакционная проверка:
  - на ясность для команды;
  - на фактологическую аккуратность;
  - на отсутствие неподтвержденных обещаний.
- За этот слой отвечает `editorial-quality-audit`; он не подменяет UX-sign-off и не отменяет glossary/doc sync проверки.
...
- Канонический порядок top-level карточки:
  ...
  5. `Память`
  6. `Риски`
- В шапке карточки обязательно показывать тип агента в user-facing терминах:
  `Автономный агент` или `Процессный агент` (без raw `core/specialist`).
- Для типа агента обязателен tooltip с простым объяснением роли:
  автономный — самостоятельная роль; процессный — sub-agent внутри orchestration.
...
- `editorial-quality-audit` является обязательным specialist-review для обзорных документов, описаний системы, agent descriptions, section descriptions и поясняющего текста в UI.
- Границы ответственности обязательны:
  - `designer-agent` отвечает за UX и восприятие интерфейса;
  - `editorial-quality-audit` отвечает за ясность, фактологию и безопасные обещания текста;
  - `terminology-consistency-audit` отвечает за канонические термины;
  - `docs-spec-sync` отвечает за согласованность source-of-truth документов.
...
- Этот UX-гейт для `designer-agent` включает 5 обязательных проверок:
  ...
  - консистентность состояний и типографики;
  - пояснения в точках риска (tooltip/inline-help);
  - защита рискованных действий (подтверждения и текст последствий).
- В проверке `консистентность состояний и типографики` обязателен единый ритм
  для однотипных элементов интерфейса: `font-size`, `font-weight`, `line-height`.
...
- Для unified drawer раздел `Долговременная память` должен показывать только memory-артефакты:
  - lessons текущего агента;
  - глобальные lessons/knowledge-артефакты, если они реально доступны в индексе;
  - постоянные правила и operating standards не показываются в `Долговременная память` и остаются в разделах `Навыки и правила` / operating context.
- Для `Оперативная память` и `Долговременная память` строка статистики обязательна и показывается в формате:
  - `операций: N read: R write: W delete: D`
  - значения считаются по фактически отображаемому набору trace/entries, а не по статическим заглушкам.
