...
- Для modern-агентов (`analyst-agent`, `designer-agent`) используется единый `UnifiedAgentDrawer`;
  special-case drawer для конкретного modern-агента не является каноническим решением.
