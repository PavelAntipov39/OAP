...
## Быстрый обзор для команды
- Понятное краткое описание того, как работает OAP: [PROJECT_OVERVIEW.md](/Users/pavelantipov/Downloads/VS%20Code/ОАП/docs/subservices/oap/PROJECT_OVERVIEW.md)
...
- Роль.
- Текущий статус.
- Тип агента (`Автономный агент` или `Процессный агент`) с tooltip-пояснением роли в системе.
- Обновлено.
- Источник данных.
...
- Для пункта `консистентность состояний и типографики` обязателен единый ритм
  однотипных UI-элементов: `font-size`, `font-weight`, `line-height`.
