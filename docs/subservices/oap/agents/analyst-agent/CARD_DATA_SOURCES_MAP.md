# Analyst Agent Card Data Sources Map

Этот документ является каноническим successor для legacy-файла `CARD_FULL_FLOW.md`.

Назначение:
- зафиксировать data-source map analyst-card;
- отделить source mapping от process-model naming;
- дать один стабильный вход для routing задач, которые меняют происхождение полей карточки.

Legacy compatibility:
- старый путь `docs/subservices/oap/agents/analyst-agent/CARD_FULL_FLOW.md` сохраняется как compatibility alias;
- source-of-truth для analyst-card data source mapping переносится в текущий файл.

## Что описывает документ
- какие поля карточки существуют;
- какие файлы и артефакты их наполняют;
- где агент читает и где пишет;
- как разделены краткосрочная и долговременная память;
- в каком шаге цикла обновляются `tasks`, `taskEvents`, `improvements`, telemetry и notifications.

## Быстрая карта полей карточки
...
- `operatingPlan`: `docs/agents/registry.yaml` + `docs/subservices/oap/agents/analyst-agent/OPERATING_PLAN.md`.
- `tasks` / `taskEvents`: `docs/agents/registry.yaml` + telemetry overlay.
- `memoryContext`:
...
- `improvements[]`: `docs/agents/registry.yaml`.
- telemetry:
...
- Для session/file trace используется двухуровневая модель:
  - `source_kind`: класс источника (`registry`, `template_catalog`, `operating_plan`, `spec`, `contract`, `telemetry_log`, `generated_artifact`, `capability_snapshot`, `unknown`);
  - `semantic_layer`: предметный слой (`skills`, `tools`, `mcp`, `rules`, `tasks`, `memory`, `schema`, `telemetry`, `unknown`).
- `source_kind` отвечает на вопрос "что это за файл/артефакт".
- `semantic_layer` отвечает на вопрос "что именно агент в нем искал или обновлял".
- Эти измерения нельзя смешивать в один UI-label.
- Step-aware `semantic_layer` policy:
...
- Consistency guard (soft-warning):
  ...
  - warning при `>10%`;
  - цикл не блокируется, warning публикуется в `artifacts/agent_telemetry_summary.json` и `.md`.
- Для analyst-card и `#/agent-flow` целевой контракт telemetry-артефакта:
...
- Backward compatibility:
  - legacy формат `artifacts_read[] = ["path"]` и `artifacts_written[] = ["path"]` остается допустимым;
  - при legacy-формате UI должен восстанавливать только `source_kind`, если это можно сделать без домыслов;
  - `semantic_layer` в legacy-режиме считается `unknown`, если он не пришел из telemetry или контракта шага.
...
views {
  ...
  view analyst_flow_steps of oap_card_flow {
    ...
    include analyst, owner, process_cycle, sec_overview, sec_mcp, sec_skills_rules, sec_tasks_quality, sec_memory_context, sec_improvements, ui_card
    autoLayout TopBottom
  }

  view analyst_flow_io of oap_card_flow {
    title "Analyst Card Flow - File I/O and Artifacts"
    include analyst, process_cycle, registry_store, spec_store, skill_store, telemetry_tool, telemetry_log_store, telemetry_report_store, content_builder, schema_guard, generated_store, sec_tasks_quality, sec_improvements
    autoLayout LeftRight
  }

  view analyst_flow_notifications of oap_card_flow {
    title "Analyst Card Flow - Security and Notifications"
    include analyst, owner, process_cycle, notifier, env_secrets, telegram_api, telemetry_tool, telemetry_log_store, telemetry_report_store
    autoLayout LeftRight
  }
}
```
